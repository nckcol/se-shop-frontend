...
export function request(op, parameters, attempt) {
  ...
  return acquireRights(op, spec, options).then(rights => {
    ...
    return makeFetchRequest(req)
      .then(
        res => processResponse(req, res, attempt, options),
        e => processError(req, e)
      )
      .then(
        outcome =>
          outcome.retry ? request(op, parameters, attempt + 1) : outcome.res
      );
  });
}
  ...
  if (op.security && options.getAuthorization) {
    return op.security.reduce((promise, security) => {
      return promise.then(rights => {
        ...
        return options
          .getAuthorization(security, securityDefinition, op)
          .then(auth => {
...
function groupParams(op, parameters) {
  const groups = ['header', 'path', 'query', 'formData'].reduce(
    (groups, name) => {
...
function buildUrl(op, baseUrl, parameters, spec) {
  ...
  if (parameters.path) {
    url = Object.keys(parameters.path).reduce(
      (url, name) => url.replace(`{${name}}`, parameters.path[name]),
      url
    );
  }
...
function buildHeaders(op, parameters) {
  ...
  else if (spec.contentTypes && spec.contentTypes[0])
  ...
  Object.keys(rights).forEach(name => {
    ...
    switch (definition.type) {
      case 'basic':
        ...
        break;
      case 'oauth2':
        req.header[resolveAuthHeaderName('Authorization')] = `Bearer ${
          rightsInfo.token
        }`;
        break;
      case 'apiKey':
        if (definition.in === 'header') {
          req.header[resolveAuthHeaderName(definition.name)] =
            rightsInfo.apiKey;
        } else if (definition.in === 'query') {
          req.query[definition.name] = rightsInfo.apiKey;
        } else {
          throw new Error(
            `Api key must be in header or query not '${definition.in}'`
          );
        }
        break;
      default:
        throw new Error(
          `Security definition type '${definition.type}' not supported`
        );
    }
  });
  return req;
...
function processResponse(req, response, attempt, options) {
  ...
  return parse.then(data => format(response, data, options)).then(res => {
    if (options.processResponse)
      return options.processResponse(req, res, attempt);
...
export function formatArrayParam(array, format, name) {
  if (!array) return;
  if (format === 'multi')
    return array.map(value => createQueryParam(name, value));
